...
int main(void) {
    DgDiagnostics d={0}; uint8_t r[96]={0};
    word(r,0,DG_GL_DESKTOP);word(r,32,DG_DESKTOP_READBACK);word(r,48,3);word(r,52,5);
    dg_diagnostic_record(&d,r);
    assert(d.records==1 && d.desktop[DG_DESKTOP_READBACK]==1 && d.desktop_bytes[DG_DESKTOP_READBACK]==60);
    word(r,0,DG_GL_QUERY);word(r,32,809);
    for (unsigned i=0;i<DG_DIAGNOSTIC_QUERIES+1;++i) { word(r,36,i);dg_diagnostic_record(&d,r); }
    assert(d.query_count==DG_DIAGNOSTIC_QUERIES && d.query_overflow==1);
    word(r,36,0);dg_diagnostic_record(&d,r);assert(d.queries[0].count==2 && d.query_overflow==1);
    word(r,0,DG_GL_CALL);word(r,32,DG_DIAGNOSTIC_FUNCTIONS);dg_diagnostic_record(&d,r);
    assert(d.function_overflow==1 && d.functions[809]==DG_DIAGNOSTIC_QUERIES+2);
